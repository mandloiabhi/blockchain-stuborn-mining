                ...
                if self.lead==-1:
                    ...
                    for i in self.adja:
                            if blk1.bkid not in global_data.block_id_list[i]:
                                ...
                                ev=Event(self.id1,i,'BLK_REC',ti,blk1,blk1.parent_bkid)
                                temp_event_list.append(ev)
                    self.public_chain=list(self.private_chain[:-1]) # as the private chain becomes public chain now
                    #self.private_chain.clear()
                    self.lead=1
                    for j in self.public_chain:
                        print(j.bkid)
                    print("public  chain is finished and lead becomes 1")
                else:
        ...
        if flag==1:
            ...
            if len(x)>0:
                ...
                while len(broadcast_block_list)>0:
                    ...
                    if block1.bkid==block12.bkid:
                        print("wrong broadcast for ",block1.bkid)
                    for i in self.adja:
